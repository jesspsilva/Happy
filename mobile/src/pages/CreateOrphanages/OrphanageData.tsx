...
export default function OrphanageData() {
    ...
    async function handleCreateOrphanage(){
        ...
        navigation.navigate('CreateOrphanageOk');
    }

    function handleErrors(){
        if((name && about && instructions && opening_hours && whatsapp_number !== '') && (latitude && longitude !== 0) && (images.length > 0)){
            setErrors(false);
    ...
    async function handleSelectedImages(){
        ...
        if(status != 'granted'){
            alert('Deve permitir o acesso à galeria');
            return;
        }
    ...
    <ScrollView style={styles.container} contentContainerStyle={{ padding: 24 }}>
        ...
        <View style={styles.switchContainer}>
            <Text style={styles.label}>Aberto ao fim de semana?</Text>
            <Switch 
                thumbColor="#fff" 
        ...
        <View style={styles.error}>
            {errors && (
                <Text style={styles.show}>Os campos são de preenchimento obrigatório</Text>
            )}
        </View>
        <RectButton style={styles.nextButton} onPress={handleErrors}>
            <Text style={styles.nextButtonText}>Submeter</Text>
        </RectButton>
    </ScrollView>
    )
}
